 ...
 * servers and databases. Copyright (C) 2008 Kevin Darty, Alain Denise and Yann
 * Ponty. electronic mail : Yann.Ponty@lri.fr paper mail : LRI, bat 490
 * Université Paris-Sud 91405 Orsay Cedex France
 *
 * This file is part of VARNA version 3.1. VARNA version 3.1 is free software:
 ...
 * VARNA version 3.1. If not, see http://www.gnu.org/licenses.
 */
...
import fr.orsay.lri.varna.models.naView.*;
...
public class NAView {
    ...
    double HELIX_FACTOR = 0.7;
    double BACKBONE_DISTANCE = 27;
    
    public static void main(String [] args)
    {
        int [] paired = RNAFoldingTools.getPairedSitesFromDotBracketString("((((.....))))...((..))");
        
        //NAView na = new NAView();
        System.out.println(NAView.naview_xy_coordinates(paired));
    }

    public static ArrayList<Point2D.Double> naview_xy_coordinates(int[] pairedSites) {
        
        ArrayList<Short> pairs = new ArrayList<Short>();
        for (int i = 0; i < pairedSites.length; i++) {
            pairs.add(new Short((short) (pairedSites[i] - 1)));
        }

        ArrayList<Double> x = new ArrayList<Double>();
        ArrayList<Double> y = new ArrayList<Double>();
        try {
            new NAView().naview_xy_coordinates(pairs, x, y);
        } catch (ExceptionNAViewAlgorithm ex) {
            Logger.getLogger(NAView.class.getName()).log(Level.SEVERE, null, ex);
        }
            ...
            if ((mate = bases.get(i).getMate()) != 0 && !mark.get(i)) {
                ...
                if (debug) {
                    ...
                    System.out.printf(
                            ...
                            regions.get(nregion).getEnd1(), regions.get(nregion).getStart2(), regions.get(
                            nregion).getEnd2(), regions.get(nregion).getStart2()
...
//			retloop.getConnections().add(new Connection());
    ...
    private void traverse_loop(Loop lp, Connection anchor_connection)
        ...
        while (true) {
            ...
            if (anchor_connection == null) {
                ...
                xo = (bases.get(acp.getStart()).getX() + bases.get(acp.getEnd()).getX()) / 2.0;
                yo = (bases.get(acp.getStart()).getY() + bases.get(acp.getEnd()).getY()) / 2.0;
    ...
    private void find_center_for_arc(double n, double b)
        ...
        if (b < 1) // otherwise we might fail below (ih)
        {
            hlow = 0;
        }
        iter = 0;
